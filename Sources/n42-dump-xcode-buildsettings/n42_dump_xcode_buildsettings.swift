...
struct n42_dump_xcode_buildsettings {
    ...
    private static func printUsage() {
        print("Usage: n42-dump-xcode-buildsettings")
        print("Runs xcodebuild settings dump, sanitizes volatile values, and writes per-target JSON files to PersistedLogs/buildConfigs.")
    }
}

enum BuildSettingsTool {
    static let outputDirectory = URL(fileURLWithPath: "PersistedLogs/buildConfigs", isDirectory: true)
    static let combinedOutputFileName = "allTargets.json"
    ...
    static func run(fileManager: FileManager = .default) throws {
        let combinedOutputURL = outputDirectory.appendingPathComponent(combinedOutputFileName)
